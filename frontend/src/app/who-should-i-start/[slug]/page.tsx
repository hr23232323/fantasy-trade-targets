...
function PlayerCard({ page, side, recommended }: { page: NonNullable<ReturnType<typeof getPlayerPage>>; side: StartSitDecision["left"]; recommended: boolean }) {
  return <article className={`relative overflow-hidden border border-[#171c19] p-6 shadow-[5px_5px_0_#171c19] ${recommended ? "bg-[#dfff4f]" : "bg-white"}`}>
    ...
    <div className="grid grid-cols-[112px_1fr] items-end gap-5"><div className="relative h-[148px] w-[112px]"><PlayerPortrait slug={page.slug} name={page.name} image={page.image} position={side.position} team={side.team} variant="card" sizes="112px" /></div><div><h2 className="text-3xl font-black leading-none tracking-[-0.05em]">{page.name}</h2><div className="mt-3 flex items-center gap-2"><TeamLogo team={side.team} size={32} /><span className="font-mono text-xs font-bold">{side.team ?? "—"} vs. {side.opponent ?? "TBD"}</span></div></div></div>
