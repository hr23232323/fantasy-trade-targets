...
import { activeStartSitWeek, getStartSitDecisions, startSitComparisons, startSitPath } from "../lib/start-sit";
...
export const metadata: Metadata = {
  ...
  description: `Week ${activeStartSitWeek} fantasy football start/sit answers for 20 close lineup decisions. Compare Standard, Half PPR and PPR using usage, matchup and availability.`,
  alternates: { canonical: PATH },
  openGraph: { type: "website", url: PATH, title: `Who Should I Start in Week ${activeStartSitWeek}?`, description: `Twenty evidence-backed Week ${activeStartSitWeek} fantasy football lineup decisions.` },
};
  ...
  return (
    <>
      ...
      <section className="page-wrap py-14"><div className="mb-8 max-w-3xl"><span className="eyebrow">Week {activeStartSitWeek} decision board</span><h2 className="section-title mt-6">Start the stronger profile. Inspect every input.</h2><p className="mt-5 text-sm leading-7 text-[#59605c]">The number shown is the Half PPR median estimate. Open a decision for floor and ceiling ranges, all three reception formats, usage, snap share and availability.</p></div><div className="grid gap-5 lg:grid-cols-2">{cards.map(({ comparison, decision }) => {
        ...
        return <Link key={comparison.slug} href={startSitPath(comparison.slug)} className="group border border-[#171c19] bg-white p-6 shadow-[5px_5px_0_#171c19] transition-transform hover:-translate-y-1"><div className="flex items-center justify-between gap-4"><span className="eyebrow bg-[#dfff4f]">{comparison.position} · {decision.winner.close ? "Close call" : `${decision.winner.gap.toFixed(1)}-point lean`}</span><span className="font-mono text-xs font-black">{winner.projection.median.toFixed(1)}–{other.projection.median.toFixed(1)}</span></div><h3 className="mt-6 text-2xl font-black leading-tight tracking-[-0.04em]">Start {winner.name} <span className="text-[#69706c]">over {other.name}</span></h3><div className="mt-5 flex flex-wrap items-center gap-4 text-xs"><span className="flex items-center gap-2"><TeamLogo team={winner.team} size={28} />{winner.team} vs. {winner.opponent}</span><span>{winner.projection.availability}</span><span className="font-mono font-bold uppercase group-hover:underline">Open decision →</span></div></Link>;
      })}</div></section>
