...
export function getPickMarket({
  ...
  return {
    assets: response.data.map((pick): MarketAsset => ({
      ...
      position: "PICK",
      kind: "pick",
      value: Math.max(0, Math.min(1000, pick.composite)),
      year: pick.year,
      round: pick.round,
      slot: pick.slot,
      tier: pick.tier,
...
export async function getMarket(
  ...
  const basePlayers: MarketAsset[] = playersResponse.data.map((player) => ({
    ...
    kind: "player",
    value: Math.max(0, Math.min(1000, player.composite)),
    team: player.team,
    age: player.age,
    rank: player.rank,
...
export async function getPlayerProfile(
  ...
  return {
    data: {
      ...
      kind: "player",
      value: Math.max(0, Math.min(1000, response.data.composite ?? response.data.value)),
      history: response.data.history ?? [],
      similar: (response.data.similar ?? []).map((player) => ({
        ...player,
        id: player.slug,
        kind: "player" as const,
        value: Math.max(0, Math.min(1000, player.composite ?? player.value)),
      })),
    },
