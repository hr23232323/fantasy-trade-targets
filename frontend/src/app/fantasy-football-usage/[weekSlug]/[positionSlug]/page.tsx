      ...
      <section className="page-wrap py-14">
        <div className="overflow-x-auto border border-[#171c19] bg-white/55">
          <table className="w-full min-w-[960px] text-left text-sm">
            ...
            <tbody className="divide-y divide-[#bcb9ae]">{rows.slice(0, 50).map((row) => <tr key={row.slug}><td className="p-4 font-mono font-black">#{row.rank}</td><td className="p-4"><div className="font-black">{hasPlayerPage(row.slug) ? <Link href={`/players/${row.slug}`} className="hover:underline">{row.name}</Link> : row.name}</div><span className="mt-1 block text-xs text-[#69706c]">{row.team ?? "—"} · Redraft rank {row.marketRank ?? "—"}</span></td><td className="p-4 font-mono">{row.opponent ? `vs. ${row.opponent}` : "—"}</td><td className="p-4 font-mono font-black">{formatPct(row.snapPct)}</td><td className={`p-4 font-mono font-black ${deltaClass(row.snapDelta)}`}>{formatPctDelta(row.snapDelta)}</td><td className="p-4 font-mono font-black">{row.volume}</td><td className={`p-4 font-mono font-black ${deltaClass(row.volumeDelta)}`}>{formatNumberDelta(row.volumeDelta)}</td><td className="p-4 font-mono">{row.halfPprPoints?.toFixed(1) ?? "—"}</td></tr>)}</tbody>
          </table>
        </div>
